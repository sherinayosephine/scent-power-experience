...
export default function NoteSelection() {
  ...
  useEffect(() => {
    ...
    if (allTried) {
      ...
      // Pastikan note yang direkomendasikan belum pernah dicoba
  ...
  return (
    <div className="h-screen w-full bg-black text-white p-6 md:px-12 md:py-8 flex flex-col items-center relative overflow-hidden">
      ...
      {/* YSL Logo */}
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="absolute top-8 right-12 w-14 z-30">
      ...
      <header className="text-center mt-10 mb-4 shrink-0">
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-3xl md:text-5xl font-bold tracking-widest uppercase">
          Discover Your New Powers
        </motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-[10px] md:text-xs tracking-[0.6em] text-gray-500 uppercase mt-3">
          YSL Les Pouvoirs de Sillage
        </motion.p>
      </header>

      {/* 2. Grid Section */}
      <div className="w-full max-w-6xl mt-16 md:mt-20 shrink-0">
        <div className="grid grid-cols-6 gap-2 md:gap-4 border border-white/10 p-3 md:p-6 bg-white/5 backdrop-blur-sm relative">
          {LAYERING_NOTES.map((note, index) => {
            ...
            return (
              ...
              >
                ...
                <p className={`text-[9px] md:text-xs tracking-[0.2em] uppercase font-bold ${isActive ? "text-white" : "text-gray-600"}`}>
                  {note.name}
                </p>
              </motion.div>
            );
      ...
      <div className="flex-1 flex flex-col justify-start mt-12 md:mt-20 w-full max-w-2xl min-h-0">
        <AnimatePresence mode="wait">
          {!isHighlighting && (
            <motion.div key={currentNote.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="text-center">
              
              <div className="bg-white/10 border border-white/20 p-6 mb-8 backdrop-blur-md rounded-sm">
                ...
                <p className="text-xs md:text-sm text-gray-400 tracking-wide leading-relaxed font-medium">
              ...
              <button
                ...
                className="bg-white text-black px-12 md:px-16 py-4 md:py-5 text-xs md:text-sm tracking-[0.4em] uppercase hover:bg-gray-200 transition-all font-bold shadow-xl active:scale-95 w-full md:w-auto"
              >
                Experience Note
              </button>
            </motion.div>
